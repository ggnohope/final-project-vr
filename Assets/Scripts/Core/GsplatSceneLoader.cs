...
namespace Core
{
    public class GsplatSceneLoader : MonoBehaviour
    {
        ...
        public void LoadScene(string regionId, string plyPath, CameraConfig cameraConfig)
        {
            if (IsLoading)
            {
                Debug.LogWarning($"[GsplatSceneLoader] Already loading a scene. Ignoring request for {regionId}");
                return;
            }
        ...
        private IEnumerator LoadPlyAssetCoroutine(string plyPath)
        {
            ...
            {
                Debug.LogError($"[GsplatSceneLoader] Failed to load asset at path: {plyPath}");
                yield break;
            }
        ...
        private void ApplyCameraConfiguration(CameraConfig config)
        {
            if (mainCamera == null)
            {
                Debug.LogWarning("[GsplatSceneLoader] Main camera is null, cannot apply camera configuration");
                return;
            }
